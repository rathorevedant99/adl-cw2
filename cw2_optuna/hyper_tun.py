...
def main():
    ...
    args = parse_args()
    if args.mode == 'train':
        study = optuna.create_study(storage = "sqlite:///optuna_pets.db", study_name="pet_segmentations", direction='minimize', sampler=optuna.samplers.TPESampler(), pruner=optuna.pruners.MedianPruner(n_warmup_steps=2))
